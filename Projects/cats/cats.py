...
def accuracy(typed, source):
    ...
    if typed_words == source_words:
        return 100.0
    if len(typed_words) == 0:  #in case the return value DIVIDE 0
        return 0.0
    count = 0    
    length = min(len(typed_words), len(source_words))   #in case index out of range
    for j in range(length):
        if typed_words[j] == source_words[j]:
            count += 1
...
def furry_fixes(typed, source, limit):
    ...
    #"To ensure that you are correctly saving time by stopping the recursion after limit is reached"
    ...
    dif_length = abs(len(typed) - len(source))

    def f(s, t, dif, pos):
        if pos == n or dif > limit:
            return dif
        if s[pos] != t[pos]:
            return f(s, t, dif + 1, pos + 1)
        else:
            return f(s, t, dif, pos + 1)
        
    return f(typed, source, 0, 0) + dif_length
    '''
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed and source and typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        add, mov, sub = float('inf'), float('inf'), float('inf')            #infinite
        if typed:
            mov = minimum_mewtations(typed[1:], source, limit - 1) + 1      #delete
        if source:
            add =  minimum_mewtations(typed, source[1:], limit - 1) + 1     #add
        if typed and source:
            sub=  minimum_mewtations(typed[1:], source[1:], limit - 1) + 1  #substitute

    return min(add, mov, sub)
...
def report_progress(typed, source, user_id, upload):
    ...
    d = {"id": user_id, "progress": progress}   #<lambda>() takes 1 positional argument but 2 were given
    upload(d)
    print(progress)
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_timestamps in tpp:
        player_times = []
        for j in range(len(player_timestamps) - 1):
            player_times.append(player_timestamps[j + 1] - player_timestamps[j])
...
def check_words_and_times(words_and_times):
    ...
    words, times = words_and_times['words'], words_and_times['times']
    assert all([type(w) == str for w in words]), "words should be a list of nstrings"
